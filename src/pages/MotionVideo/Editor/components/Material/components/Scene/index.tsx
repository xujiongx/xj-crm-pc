import { VIEWPORT_SIZE } from '@/pages/MotionVideo/Editor/hooks/useViewportSize';
...
import { SlideItem } from '@/pages/MotionVideo/Editor/interface';
...
const SingleSlide = ({ slideInfo }: { slideInfo: SlideInfo }) => {
  let { slide, width, viewportRatio, slideIndex, index, updateSlideIndex, CONTEXTMENU_Ele, slidesWrapper, contextMenuClickFn, hanldefocusFn } = slideInfo
  return (
    <div
      key={slide.id}
    ...
    >
      <ScreenView
        ...
        slide={slide}
      />
      <ContextMenu MenuItem={CONTEXTMENU_Ele} targetEl={slidesWrapper.current?.childNodes?.[1].childNodes[index] as HTMLDivElement} contextMenuClickFn={contextMenuClickFn} defaultAction={() => { console.log('当前 index', index); hanldefocusFn(index) }}></ContextMenu>
    </div>
  )
}
  ...
  return (
    <div>
      {props.slides.map((slide: SlideItem, index: number) => (
        <SortableSingleSlide key={slide.id} index={index} slideInfo={{ ...props, slide, index }} />
      ))}
    </div>
  )
}

const SortableSlideList = SortableContainer(SlideList)

const SceneMaterial = () => {
  const sizeRef = useRef<HTMLDivElement>(null);
  const slidesWrapper = useRef<HTMLDivElement>(null);
  const { width = 0 } = useSize(sizeRef) || {};
  const { slides, slideIndex, addSlide, updateSlideIndex, currentSlide, deleteSlide, setSlides } = useSlidesStore(
    (state) => state,
  );
  ...
  const slideDragEnd = function ({ oldIndex, newIndex }: SortEnd) {
    if (oldIndex === newIndex || !Number.isInteger(oldIndex) || !Number.isInteger(newIndex)) return
  ...
  return (
    <div className={styles['slide-wrapper']}>
      ...
      <div className={styles.slides} ref={slidesWrapper}>
        ...
        <SortableSlideList onSortEnd={slideDragEnd} distance={10} slides={slides} width={width} viewportRatio={viewportRatio} slideIndex={slideIndex} updateSlideIndex={updateSlideIndex} CONTEXTMENU_Ele={CONTEXTMENU_Ele} slidesWrapper={slidesWrapper} contextMenuClickFn={contextMenuClickFn} hanldefocusFn={hanldefocusFn} />
      </div>
    </div>
  );
};

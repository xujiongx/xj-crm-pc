...
const RichTextBase = () => {
  ...
  const emitRichTextCommand = (command: string, value?: string) => {
    emitter.emit(EmitterEvents.RICH_TEXT_COMMAND, {
      ...
      target: activeElementId,
    });
  };

  const bulletListStyleTypeOption = ['disc', 'circle', 'square'];
  const orderedListStyleTypeOption = [
    'decimal',
    'lower-roman',
    'upper-roman',
    'lower-alpha',
  ...
  return (
    <div className="rich-text-base">
      ...
      <ButtonGroup className="row" passive>
        <TextColorButton style={{ width: '30%' }} color={richTextAttrs.color}>
          <ColorPicker
            ...
            onChange={(value) =>
              emitRichTextCommand('color', value.toRgbString())
            }
          >
        ...
        >
          <ColorPicker
            ...
            onChange={(value) =>
              emitRichTextCommand('backcolor', value.toRgbString())
            }
          >
      ...
      <ButtonGroup className="row">
        ...
        >
          <ActionIcon icon={<Code />} tooltip="行内代码" />
        </CheckboxButton>
        ...
        >
          <ActionIcon icon={<Quote />} tooltip="引用" />
        </CheckboxButton>
      </ButtonGroup>
      ...
      <ButtonGroup className="row" passive>
        ...
        >
          <ActionIcon icon={<Clear />} tooltip="清除格式" />
        </CheckboxButton>
      ...
      <div className="row">
        <ButtonGroup style={{ flex: 1 }}>
          <Button
            first
            style={{ flex: 1 }}
            onClick={() => emitRichTextCommand('bulletList')}
          >
            <ActionIcon icon={<UnorderedListOutlined />} tooltip="项目符号" />
          </Button>
          <Popover
            placement="bottom"
            trigger="click"
            content={
              <div className="list-wrap">
                {bulletListStyleTypeOption.map((item) => (
                  ...
                  >
                    {new Array(3).fill('').map((key, index) => (
        ...
        <ButtonGroup style={{ flex: 1 }} passive>
          <Button
            ...
            onClick={() => emitRichTextCommand('orderedList')}
          >
            <ActionIcon icon={<OrderedListOutlined />} tooltip="编号" />
          </Button>
          <Popover
            trigger="click"
            placement="bottom"

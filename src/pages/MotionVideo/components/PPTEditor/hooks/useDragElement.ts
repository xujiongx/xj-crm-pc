...
const useDragElement = (
  elements: PPTElement[],
  alignmentLines: (item: AlignmentLineProps[]) => void,
) => {
  ...
  const shiftKeyState = useKeyboardStore((store) => store.shiftKeyState);
  ...
  const drag = (
    ...
    if (!activeElementIds.includes(element.id)) return;
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      // 其他被选中的元素的位置信息通过正在操作的元素的移动偏移量来进行计算
        ...
        const handleElement = elements.find((el) => el.id === element.id);

...
import {
  ...
  createSlideIdMap,
  getElementRange,
} from '@/pages/MotionVideo/utils/element';
...
const useAddSlidesOrElements = () => {
  ...
  const addElementsFromData = (elements: PPTElement[]) => {
    ...
    const elIds = elements.map((item) => item.id);

    const addElementsAnimations = currentSlide.animations
      ?.filter((animation) => elIds.includes(elIdMap[animation.elId]))
      .map((item) => ({
        ...item,
        elId: elIdMap[item.elId],
        id: uid(),
      }));
    addElement(elements, addElementsAnimations);

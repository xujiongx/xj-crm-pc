...
export default () => {
  ...
  const getSlidesThemeStyles = (slide: SlideItem | SlideItem[]) => {
    ...
    for (const slide of slides) {
      if (slide.background) {
        if (slide.background.type === 'solid' && slide.background.color) {
          ...
          slide.background.gradientColor
        ) {
          backgroundColorValues.push(
            ...slide.background.gradientColor.map((item) => ({
              area: 1,
              value: item,
            })),
          );
        } else
          backgroundColorValues.push({ area: 1, value: theme.backgroundColor });
      }
  ...
  // 获取指定幻灯片内所有颜色（主要的）
  const getSlideAllColors = (slide: SlideItem) => {
    const colors: string[] = [];
    for (const el of slide.elements) {
  ...
  // 设置幻灯片主题
  const setSlideTheme = (slide: SlideItem, theme: PresetTheme) => {
    ...
    for (const el of slide.elements) {
      ...
      if (el.type === 'table') {
        if (el.theme)
          el.theme.color =
            colorMap[tinycolor(el.theme.color).toRgbString()] || el.theme.color;
  ...
  const applyPresetThemeToSingleSlide = (theme: PresetTheme) => {
    const newSlide: SlideItem = JSON.parse(JSON.stringify(currentSlide));
  ...
  const applyPresetThemeToAllSlides = (theme: PresetTheme) => {
    const newSlides: SlideItem[] = JSON.parse(JSON.stringify(slides));
  ...
  const applyThemeToAllSlides = () => {
    const newSlides: SlideItem[] = JSON.parse(JSON.stringify(slides));
    const { themeColor, backgroundColor, fontColor, fontName } = theme;

...
import { ImageClipedEmitData } from '@/pages/MotionVideo/types/edit';
import {
  ImageElementClip,
  PPTImageElement,
} from '@/pages/MotionVideo/types/slides';
...
const ImageElement = ({ element, onSelect, store }: ImageElementProps) => {
  ...
  const setClipingImageElementId = useMainStore(
    (store) => store.setClipingImageElementId,
  );

  const updateElement = useSlidesStore((store) => store.updateElement);

  const addHistorySnapshot = useHistorySnapshot().add;

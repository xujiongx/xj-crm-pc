...
function useGetState<S>(
  initialState: S | (() => S),
): [S, SetStateAction<S>, GetStateAction<S>];
...
function useGetState<S>(initialState?: S) {
  ...
  const setState = useCallback<SetStateAction<S | undefined>>(
    (stateOrAction) => {
      const newState = isFunction(stateOrAction)
        ? stateOrAction(stateRef.current)
        : stateOrAction;
